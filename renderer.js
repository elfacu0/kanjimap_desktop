...
//   console.log("UpdateKanjiInfo");
  ...
  var KanjiAnimationPath =
    basepath + "/resources/strokeanimations/" + hexcode + "_anim.svg";
...
document.addEventListener("DOMContentLoaded", function () {
  var cy;
  var graphP = $.ajax({
    url: basepath + "/resources/json/kanjidic.json",
    type: "GET",
    dataType: "json",
  });
  var styleP = $.ajax({
  ...
  var radicaldic = $.ajax({
    url: basepath + "/resources/json/radicals.json",
    type: "GET",
    dataType: "json",
  });
  var jukugodic = $.ajax({
    url: basepath + "/resources/json/jukugo.json",
    type: "GET",
    dataType: "json",
    success: function () {},
  });
  ...
  //   console.log("initCy")
    ...
    var expJson = then[0];
    var styleJson = replaceAll(then[1], '/resources', basepath + '/resources')
    ...
    $("#searchBar").keyup(function () {
      ...
      $.getJSON( basepath + "/resources/json/kanjidic.json", function (DATA) {
...
//   // $('#paypal').on('click', () => {
//   console.log("kachin");
//   shell.openExternal('https://www.paypal.com/cgi-bin/webscr?cmd=_s-xclick&hosted_button_id=P46GSGHPZUBW2&source=url');
// })
// }
...
function openCytoscapeURL() {
  shell.openExternal('http://js.cytoscape.org/');
}

function openMITURL() {
  shell.openExternal("https://opensource.org/licenses/MIT");
}

function openKanjiVGURL() {
  shell.openExternal("http://kanjivg.tagaini.net/");
}

function openCCURL() {
  shell.openExternal("https://creativecommons.org/licenses/by-sa/3.0/");
}

function openChiseURL() {
  shell.openExternal("http://www.chise.org/ids/index.html");
}

function openGNUURL() {
  shell.openExternal("https://www.gnu.org/licenses/old-licenses/gpl-2.0.html");
}

function openAGPLURL() {
  shell.openExternal("https://www.gnu.org/licenses/agpl-3.0.en.html");
}

function openKanshudoURL() {
  shell.openExternal("https://www.kanshudo.com/");
}